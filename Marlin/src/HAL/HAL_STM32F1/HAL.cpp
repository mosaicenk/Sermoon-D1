...
volatile uint16_t HAL_adc_results[ADC_PIN_COUNT]; // Written by DMA, read by ISR — must be volatile
...
void HAL_adc_init() {
  ...
  adc.setDMA((uint16_t*)HAL_adc_results, (uint16_t)ADC_PIN_COUNT, (uint32_t)(DMA_MINC_MODE | DMA_CIRC_MODE), nullptr);  // cast away volatile for DMA API
  adc.setScanMode();
  adc.setContinuous();
  adc.startConversion();
}
