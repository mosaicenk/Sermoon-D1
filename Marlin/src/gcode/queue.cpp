  ...
  inline void GCodeQueue::get_sdcard_commands() {
    ...
    while (length < BUFSIZE && !card_eof && !stop_buffering) {
      ...
      if(card_eof)
      {
        ...
        rtscheck.RTS_SndData(100,PRINT_PERCENT_DATA_VP);
        delay(1);
        rtscheck.RTS_SndData(100 ,START_PROCESS_ICON_VP);
        delay(1);

        rtscheck.RTS_SndData(ExchangePageBase + 10, ExchangepageAddr); //切换已打印完成页面
      ...
      if (card_eof || n == -1
        ...
        sd_count = 0; // clear sd line buffer
          ...
          repeat.early_parse_M808(command_buffer[index_w]);
        #endif
