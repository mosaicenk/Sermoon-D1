...
void GcodeSuite::G28(const bool always_home_all) {
  ...
  ui.refresh();
    ...
    if(waitway == 1)
    {
      ...
      rtscheck.RTS_SndData(language_change + DWIN_STATUS_PAUSED, STATUS_DP_CHAR_VP);
  ...
  if (DEBUGGING(LEVELING)) DEBUG_ECHOLNPGM("<<< G28");
    ...
    if(waitway == 4)
    {
      if(AutohomeKey)
      {
        ...
        rtscheck.RTS_SndData(ExchangePageBase + 25 + AxisPagenum - 2, ExchangepageAddr);
      }
      waitway = 0;
    }
    else if(waitway == 6)
    {
      if(AutohomeKey)
      {
        ...
        #if HAS_LEVELING
          rtscheck.RTS_SndData(ExchangePageBase + 18, ExchangepageAddr);
        #else
          rtscheck.RTS_SndData(ExchangePageBase + 20, ExchangepageAddr);
        #endif
      }
      waitway = 0;
    }
    
    rtscheck.RTS_SndData(10 * current_position[X_AXIS], X_MOVE_DATA_KEY);
    rtscheck.RTS_SndData(10 * current_position[Y_AXIS], Y_MOVE_DATA_KEY);
    rtscheck.RTS_SndData(10 * current_position[Z_AXIS], Z_MOVE_DATA_KEY);

    #ifdef BLTOUCH
      if(card.isPrinting() && (!PoweroffContinue) && (G29_status == 0))
      {
        G29_status = 1;
