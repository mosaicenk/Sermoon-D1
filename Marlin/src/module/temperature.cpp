...
#ifdef RTS_AVAILABLE
  int room_temperature_hotend = 0;
  int room_temperature_hotbed = 0;
#endif
...
void Temperature::min_temp_error(const heater_ind_t heater) {
  ...
  float Temperature::get_pid_output_hotend(const uint8_t E_NAME) {
    const uint8_t ee = HOTEND_INDEX;
        ...
        const float pid_error_bed = (thermalManager.temp_bed.target-thermalManager.temp_bed.celsius);
        ...
        float pid_output;
          ...
          // 挤出头室温赋值
          if(!room_temperature_hotend)
          {
            //room_temperature_hotend = temp_hotend[ee].celsius;
            room_temperature_hotend = 25;
          }

          // 热床室温赋值
          if(!room_temperature_hotbed)
          {
            //room_temperature_hotbed = temp_bed.celsius;
            room_temperature_hotbed = 25;
          }

          // "加热完成"图标显示判断
          if( 
              ((pid_error <= 0)     && (PrinterStatusKey[1] == 1))  
              &&
          ...
          )  
          {
            ...
            PrinterStatusKey[1] = 0;
                    ...
                    (
                      ...
                      &&
                      ((!temp_bed.target) && (pid_error_bed >= -(room_temperature_hotbed + 10)))
                    )
                  )
          {
            Update_Time_Value = RTS_UPDATE_VALUE;
