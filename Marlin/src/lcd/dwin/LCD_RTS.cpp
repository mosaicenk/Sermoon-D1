...
#include "../../feature/bedlevel/abl/abl.h"
  ...
  void RTSSHOW::RTS_SDCardInit(void)
  {
    ...
    if(card.isMounted())
    {
      ...
      RTS_SndData(language_change + DWIN_STATUS_READY, STATUS_DP_CHAR_VP);
      msgStatus = MSG_READY;
  ...
  void RTSSHOW::RTS_SDCardUpate(void)
  {
    ...
    if (sd_status != RTS_lcd_sd_status)
    {
      if (sd_status)
      {
        delay(100);       // SD card mount settle (was 500ms, 100ms sufficient)
        card.mount();     //挂载SD卡
  ...
  void RTSSHOW::RTS_Init()
  {
    ...
    RTS_SndData(zprobe_zoffset*100, Z_OFFSET_DISPLAY_VP);
    ...
    switch(recdat.addr)
    {
      ...
      case PrintOptions: //打印界面下点击“调整”按钮 0x2007
      {
        ...
        else if(recdat.data[0] == 0x05) //在打印界面点击“继续”按钮
        {
            ...
            if(RTS_CheckFilement(0)) 
            {
              if(thermalManager.temp_hotend[0].target>= 185)
              {
                for(startprogress = 0;startprogress < 5;startprogress++)
                {
                  ...
                  delay(200);  // Filament change animation (was 400ms)
                }
              }
              else
              {
          ...
          {
            RTS_SndData(language_change + DWIN_STATUS_HEATING, STATUS_DP_CHAR_VP);// for Heating
      ...
      case TempMenu:  //点击温度选项卡进入该处理段 0x200C
      {
        ...
        else if(recdat.data[0] == 0xF2)
        {
          ...
          RTS_SndData(language_change + DWIN_STATUS_COOLING, STATUS_DP_CHAR_VP);
      ...
      case AutoZero:        //0x1046
      {
        ...
        else // AutoZero — bu case grubundaki tek kalan adres
        {
          // autohome
      ...
      case NoFilamentContinue: //断料处理
      {
        ...
        if(recdat.data[0] == 1) //断料提示界面，点击“是”，提示继续打印
        {
          ...
          else if(FilementStatus[0] == 2)
          {
            ...
            RTS_SndData(language_change + DWIN_STATUS_HEATING, STATUS_DP_CHAR_VP);
            msgStatus = MSG_HEATING;
      ...
      case PwrOffContinue:   //断电续打页面处理
      {
        ...
        else if(recdat.data[0] == 2)  //点击“否”取消断电续打
        {
          ...
          delay(200); // for system settle (was 500ms)
        }
      }
      break;
        ...
        else if(recdat.data[0] == 0x02) //在文件选中页面中，选中文件后，点击“打印”按钮
        {
          if(IS_SD_INSERTED())
          {
            ...
            RTS_SndData(ExchangePageBase + 7, ExchangepageAddr);
              ...
              RTS_SndData(0, Z_OFFSET_DISPLAY_VP);
            #endif
  ...
  void EachMomentUpdate()
  {
    ...
    if(ms > next_rts_update_ms && InforShowStatus)
    {
      ...
      // Sermoon D1 (2026-05-23): Kalan sure hesapla + DWIN'e yaz.
      // M73 komutu geldiyse (slicer'dan) onu kullan, yoksa linear extrapolation:
      //   remaining_min = elapsed_sec * (100 - pct) / pct / 60
      // pct=0 veya pct=100'de hesaplama anlamsiz; o zaman 0 yaz.
      {
        uint16_t pct = card.percentDone();
        uint16_t remain_min = 0;
        if (card.isPrinting() && pct > 0 && pct < 100) {
          uint32_t elapsed_sec = elapsed.value;
          uint32_t remain_sec   = (elapsed_sec * (100 - pct)) / pct;
          remain_min = (uint16_t)(remain_sec / 60);
          // 99 saat 59 dakika'dan sonra sinirla (uint16 ile 18 saat gosterilebilir,
          // ama DWIN ekran 4-haneli alan bekliyor; 9999 max guvenli).
          if (remain_min > 9999) remain_min = 9999;
        }
        rtscheck.RTS_SndData(remain_min, PRINT_REMAIN_MIN_VP);
      }
      ...
      if( (last_target_temperature_bed != thermalManager.temp_bed.target) || (last_target_temperature[0] != thermalManager.temp_hotend[0].target))
      {
        ...
        {
          rtscheck.RTS_SndData(language_change + DWIN_STATUS_HEATING, STATUS_DP_CHAR_VP);
          msgStatus = MSG_HEATING;
            ...
            ((last_target_temperature[0] > thermalManager.temp_hotend[0].target) && (thermalManager.temp_hotend[0].celsius > thermalManager.temp_hotend[0].target)))
        {
          if(!state_stop_flag)
          {
            rtscheck.RTS_SndData(language_change + DWIN_STATUS_COOLING, STATUS_DP_CHAR_VP);
            msgStatus = MSG_COOLING;
          }
          state_stop_flag = 0;
          Update_Time_Value = 0;
      ...
      if (msgStatus == MSG_COOLING)
      {
        ...
        {
          rtscheck.RTS_SndData(language_change + DWIN_STATUS_READY, STATUS_DP_CHAR_VP);
          msgStatus = MSG_READY;
        }
      }
  ...
  //在主循环中进行串口屏数据处理
  // Sermoon D1 optimizasyon (2026-05-23): SD card update throttling.
  // Orijinal kodda `RTS_SDCardUpate()` her main loop iteration'inda
  // cagriliyordu — SD kart polling CPU yuku. SD update'i artik
  // RTS_UPDATE_INTERVAL (1000ms) ile sinirli, display update'le
  // ayni zamanlama.
  void RTSUpdate()
  {
    static millis_t next_sd_update_ms = 0;
    const millis_t ms = millis();
    if (ms >= next_sd_update_ms) {
      rtscheck.RTS_SDCardUpate(); //检测SD卡状态
      next_sd_update_ms = ms + RTS_UPDATE_INTERVAL;
    }
