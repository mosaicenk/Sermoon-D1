...
{				  
  ...
  IIC_Send_Byte(ReadAddr%256);   //发送低地址
	IIC_Wait_Ack();
	IIC_Start();
	// 24C16'da blok-secim bitleri (B2..B0) kontrol byte'inin parcasidir ve
	// restart sonrasi OKUMA kontrol byte'inda da tekrarlanmalidir. Duz 0xA1
	// her zaman blok 0'i secerdi => 255 ustu adresler yanlis veri dondururdu.
	if(EE_TYPE>BL24C16) IIC_Send_Byte(0XA1);
...
uint8_t BL24CXX_Check(void)
{
	...
	if (checked) return 0;
	uint8_t temp = BL24CXX_ReadOneByte(255);
	if (temp == 0x55) { checked = true; return 0; }
...
{
	while (NumToRead) {
		uint16_t chunk = NumToRead;

		if (EE_TYPE > BL24C16) {
			// 16-bit adresli tipler (24C32+): blok biti yok, sinir sorunu yok
			IIC_Start();
			IIC_Send_Byte(0xA0);
			IIC_Wait_Ack();
			...
			IIC_Start();
			IIC_Send_Byte(0xA0 + block_bits);
			IIC_Wait_Ack();
			IIC_Send_Byte(ReadAddr % 256);
			IIC_Wait_Ack();
			IIC_Start();
			IIC_Send_Byte(0xA1 + block_bits);   // blok bitleri READ byte'inda da sart
			IIC_Wait_Ack();
		}

		for (uint16_t i = 0; i < chunk; i++)
			*pBuffer++ = IIC_Read_Byte((i + 1 < chunk) ? 1 : 0);
		IIC_Stop();
...
{
	while (NumToWrite) {
		uint8_t page_off = WriteAddr % EEPROM_PAGE_SIZE;
		uint8_t chunk = EEPROM_PAGE_SIZE - page_off;
		if (chunk > NumToWrite) chunk = (uint8_t)NumToWrite;
		IIC_Start();
		if (EE_TYPE > BL24C16) {
			IIC_Send_Byte(0xA0);
			IIC_Wait_Ack();
		...
		IIC_Stop();
		delay(5);  // 24C16 page-write cycle time
		WriteAddr += chunk;
		NumToWrite -= chunk;
	}
}
